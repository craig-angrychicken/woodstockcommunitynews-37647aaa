...
export interface PublishAboutPageResult {
  success: boolean;
  pageId?: string;
  url?: string;
  skipped?: boolean;
...
/**
 * Create or update the "About" page in Ghost.
 *
 * Ghost is optional: if `GHOST_API_URL` / `GHOST_ADMIN_API_KEY` are not
 * configured, this no-ops and returns a clear skipped result.
 */
export async function publishAboutPage(env: Env): Promise<PublishAboutPageResult> {
  const ghostApiKey = env.GHOST_ADMIN_API_KEY;
  let ghostApiUrl = env.GHOST_API_URL;

  if (!ghostApiKey || !ghostApiUrl) {
    console.log("ℹ️ Ghost credentials not configured — skipping about page publish");
    return { success: false, skipped: true, reason: "Ghost credentials not configured" };
  }
  ...
  try {
    ...
    if (getResponse.ok) {
      const existing = (await getResponse.json()) as { pages?: Array<{ id?: string; updated_at?: string }> };
    ...
    const result = (await response.json()) as { pages?: Array<{ id?: string; url?: string }> };
