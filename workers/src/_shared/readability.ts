...
export function extractWithReadability(html: string): ReadabilityResult {
  try {
    ...
    // @mozilla/readability expects a DOM Document; linkedom's document is compatible.
 ...
 */
export function extractImagesFromMeta(
  rawHtml: string,
  baseUrl: string
): ExtractedImage[] {
  const images: ExtractedImage[] = [];
  ...
  const pinterestRegex =
    /pinterest\.com\/pin\/create\/button\/[^"']*?media=([^&"']+)/gi;
  ...
  if (images.length === 0) {
    const finalsiteRegex =
      /https?:\/\/resources\.finalsite\.net\/images\/[^"'\s)>;,]+\.(?:jpg|jpeg|png|gif|webp)/gi;
